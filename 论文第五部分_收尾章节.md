...
| 亮点维度 | 具体体现 | 量化证据 |
|---------|---------|---------|
| **跨方法融合创新** | 约束优化与贝叶斯MCMC双路径交叉验证 | 双方法EPA均>83%，一致性46.43% |
| **多目标优化设计** | NSGA-II帕累托最优在公平性/稳定性/娱乐性间平衡 | 综合得分提升28.4% |
| **完整不确定性量化** | Bootstrap CI + 贝叶斯后验分布 | 95% CI覆盖率达95.8% |
| **数据驱动决策支持** | 基于Kendall τ的投票规则公平性度量框架 | τ差异0.14，显著性p<0.01 |
| **模型可解释性** | XGBoost + SHAP值分解 | 识别last_week贡献80.32% |
| **稳健性验证** | 10折CV、噪声敏感性、反事实模拟 | 3%噪声下R²下降仅9.73% |
...
| 序号 | 优点 | 详细阐述 | 求解数据支撑 |
|------|------|---------|-------------|
| **1** | **融合多维度特征工程，精准捕捉数据潜在规律** | 本研究构建了包含34个特征的多维特征矩阵，融合选手年龄、职业背景、专业舞伴配对、季节效应等多源信息。XGBoost模型通过特征重要性分析识别出"累积表现效应"（last_week重要度80.32%）作为核心驱动因素，揭示了DWTS比赛的马太效应规律。 | 模型拟合优度R²=0.9825，训练RMSE=0.497，成功解释98.25%的名次方差；特征工程覆盖34维变量，实现对选手表现的全方位量化建模 |
...
| **5** | **通过多重统计检验验证结论可靠性** | 模型检验环节实施10折交叉验证（CV R²=0.663±0.085）、残差分析（偏度0.32，峰度0.45）、噪声敏感性测试（3%噪声下R²下降仅9.73%）等多重检验，全面评估模型泛化能力与稳健性，确保结论可直接支撑实际决策。 | 10折CV R²标准差0.085<0.1（稳定）；Spearman ρ=0.9588，Kendall τ=0.8497（极强相关）；分类准确率75.77% |
...
| **2** | **未覆盖非结构化数据的深度挖掘** | 本研究主要基于结构化的比赛数据（评委评分、选手信息、淘汰结果）进行建模，未充分挖掘社交媒体文本、观众评论、节目视频等非结构化数据中蕴含的粉丝情感与舆情信息。（注：社交媒体粉丝量仅在辅助分析模块中用于争议案例的定性解释，不参与核心模型建模） | 辅助分析模块虽引入了社交媒体粉丝量数据，但仅作为争议案例的解释依据，未进行文本情感分析或话题热度挖掘 |
| **3** | **样本数据的时间跨度有限，可能影响预测泛化性** | 模型基于DWTS第1-34季（2005-2025年）的历史数据训练，时间跨度约20年。然而，观众偏好、社交媒体生态、真人秀节目形式均在快速演变，历史规律能否准确预测未来季节（S35+）的投票行为存在不确定性。此外，缺乏外部数据集进行独立验证。 | 结果分析模块指出研究局限："未考虑节目内容、编排等外部因素"；未来方向建议"引入时间序列分析捕捉动态效应" |
